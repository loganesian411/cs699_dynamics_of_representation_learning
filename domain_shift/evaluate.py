...
def evaluate_benchmark(
    ...
    for split in splits:
        ...
        split_subset_kwargs = additional_kwargs['subset_kwargs'][split]
        for replicate in replicates:
            ...
            if dataset_name == "globalwheat":
                ...
                metric_results = evaluate_replicate(
                    wilds_dataset, split, predicted_labels, split_subset_kwargs,
                )
            for metric in metrics:
...
def evaluate_replicate(
    ...
    """
    # Dataset will only be downloaded if it does not exist
    subset: WILDSSubset = dataset.get_subset(split, **subset_kwargs)
