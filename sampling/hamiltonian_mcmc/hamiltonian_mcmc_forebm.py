...
def euler_integration(x, v, E, M=None, eps=0.1, E_grad=None):
...
def symplectic_integration(x, v, E, M=None, eps=0.1, E_grad=None,
                           E_grad_array_wise=True):
...
def hamiltonian_mcmc(x, E, K, eps=0.1, key=jax.random.PRNGKey(_SEED),
                     ...
                     grad_func=None, apply_along_axis=True):
  ...
  for i in tqdm(range(K)):
    x, v = hamilton_ode(x, v, E, M=M, eps=eps, E_grad=grad_func,
                        E_grad_array_wise=apply_along_axis)
    key, subkey = jax.random.split(key)
    if include_kinetic:
      x, acceptance_rate = metropolis_hastings_adjustment(E, x_orig, x, key=subkey,
        v_prev=v_orig, v_new=v, M=M, apply_along_axis=apply_along_axis)
    else:
      x, acceptance_rate = metropolis_hastings_adjustment(E, x_orig, x, key=subkey,
